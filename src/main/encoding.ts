...
export function detectEncoding(buf: Buffer): EncodingId {
  ...
  if (buf.length >= 4 && buf.length % 2 === 0) {
    if (nullRatio(buf, false) > 0.3 && nullRatio(buf, true) < 0.08) return 'utf16le'
    if (nullRatio(buf, true) > 0.3 && nullRatio(buf, false) < 0.08) return 'utf16be'
  }
...
export function decodeBuffer(buf: Buffer, encoding: EncodingId): string {
  ...
  return iconv.decode(buf, iconvName(encoding))
}

export function encodeText(text: string, encoding: EncodingId): Buffer {
  const body = iconv.encode(text, iconvName(encoding))
